...
def test_gate_g1_executes_after_linear_run(tmp_path) -> None:
    run_reference_benchmark("BP_Linear_Two_Block", seed=2, root=tmp_path)
    result = evaluate_g1(tmp_path)
    assert result["gate"] == "G1"
    assert "metrics" in result


def test_gate_g3_executes_after_transport_run(tmp_path) -> None:
    run_reference_benchmark("BP_Windowed_Transport_Flow", seed=0, root=tmp_path)
    run_reference_benchmark("BP_T3_Window_Sensitivity_Pair", seed=0, parameter_id="reference", root=tmp_path)
    result = evaluate_g3(tmp_path)
...
def test_gate_g4_executes_after_nonlinear_run(tmp_path) -> None:
    run_reference_benchmark("BP_Weakly_Nonlinear_Slow_Manifold", seed=0, root=tmp_path)
    run_reference_benchmark("BP_T4_Local_Validity_Pair", seed=0, parameter_id="reference", root=tmp_path)
    result = evaluate_g4(tmp_path)
...
def test_gate_g5_reports_stochastic_stress_metrics(tmp_path) -> None:
    run_reference_benchmark("BP_Noisy_Metastable_Network", seed=0, root=tmp_path)
    run_reference_benchmark("BP_T5_Stochastic_Stress_Pair", seed=0, parameter_id="reference", root=tmp_path)
    result = evaluate_g5(tmp_path)
...
def test_gate_g6_executes_after_required_reference_runs(tmp_path) -> None:
    for benchmark_id in (
        "BP_Linear_Two_Block",
    ...
    run_reference_benchmark("BP_Clickstream_Docs_Funnel", seed=0, root=tmp_path)
    run_reference_benchmark("BP_Clickstream_Docs_Funnel", seed=0, parameter_id="negative_detour", root=tmp_path)
    run_reference_benchmark("BP_Support_Portal_Funnel", seed=0, root=tmp_path)
    run_reference_benchmark("BP_Support_Portal_Funnel", seed=0, parameter_id="negative_detour", root=tmp_path)
    run_reference_benchmark("BP_Workflow_Queue_Funnel", seed=0, root=tmp_path)
    run_reference_benchmark("BP_Workflow_Queue_Funnel", seed=0, parameter_id="negative_detour", root=tmp_path)
    result = evaluate_g6(tmp_path)
...
def test_gate_g2_reports_delay_refinement_metrics(tmp_path) -> None:
    for benchmark_id in ("BP_Non_Normal_Shear", "BP_Random_Gap_Ensemble", "BP_Delay_Coupled_Pair"):
        run_reference_benchmark(benchmark_id, seed=0, root=tmp_path)
    run_reference_benchmark("BP_T2_Same_Spectrum_Pair", seed=0, parameter_id="reference", root=tmp_path)
